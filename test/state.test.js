'use strict';
// Integration tests over the LIVE repo governance files (read-only). These
// assert structural invariants (the Brief A success criteria), not hardcoded
// counts, so they stay valid as governance content evolves.
const test = require('node:test');
const assert = require('node:assert');
const fs = require('fs');
const path = require('path');
const { buildState } = require('../src/state');
const { REPO_ROOT, extractTables, findTable } = require('../src/parse');
...
test('criterion (a)+(b): every rendered field tiered; every object carries resolvable source evidence', () => {
  ...
  for (const o of all) {
    ...
    const abs = path.join(REPO_ROOT, o.source_evidence.value.file);
...
test('criterion (c): every strategic_initiatives and weekly_priorities row renders exactly one object', () => {
  const state = buildState();
  const siContent = fs.readFileSync(path.join(REPO_ROOT, 'agents/founder/outputs/strategic_initiatives.md'), 'utf8');
  ...
  const wpContent = fs.readFileSync(path.join(REPO_ROOT, 'agents/founder/outputs/weekly_priorities.md'), 'utf8');
...
test('criterion (d): Approval Queue carries every open decision row and every conditional/pending gate', () => {
  const state = buildState();
  const dqContent = fs.readFileSync(path.join(REPO_ROOT, 'agents/founder/outputs/decision_queue.md'), 'utf8');
...
test('criterion (e): every loaded source classifies into exactly one schema family', () => {
  ...
  // the five Brief A sources are all governance per contract §8
...
test('criterion (g): building state writes zero bytes to any source file', () => {
  ...
  const before = targets.map(t => fs.statSync(path.join(REPO_ROOT, t)).mtimeMs);
  buildState();
  buildState(); // repeated pulls
  const after = targets.map(t => fs.statSync(path.join(REPO_ROOT, t)).mtimeMs);
...
test('parse-error state: a missing file degrades to an error entry, never a crash', () => {
  ...
  const f = parseFile(path.join(REPO_ROOT, 'agents/founder/outputs/does_not_exist.md'));
