'use strict';
// Brief B + Phase 0b + Goal C tests, over the live repo (read-only).
...
const { getRepoHealth, READONLY_SUBCOMMANDS } = require('../src/repohealth');
const { REPO_ROOT, parseFile } = require('../src/parse');

const TIERS = ['Canonical', 'Derived', 'Candidate'];

test('Phase 0b: all 5 AGENT.md + 7 SKILL.md carry definition frontmatter', () => {
  const defs = discoverDefinitionFiles(REPO_ROOT);
...
test('Topology: Canonical edges from frontmatter, every edge tiered + source-evidenced (Gate C Amendment 1)', () => {
  const t = buildTopology(REPO_ROOT);
  ...
  for (const e of t.edges) {
    // tier is always present (Canonical/Derived value or nys)
  ...
  // A known Canonical edge: founder owns goal-setting.
  ...
  // A known depends-on edge: priority-setting depends-on goal-setting.
...
test('Roadmap Spine: Phase nodes parsed; roadmap terms not crawl|walk|run (FR13)', () => {
  const r = buildRoadmap(REPO_ROOT);
...
test('Goal C Repo Health: read-only allowlist; returns branch + numeric counts', () => {
  // The module only ever runs these git subcommands.
...
test('Repo Health source uses no destructive git verbs (static guard)', () => {
  const src = fs.readFileSync(path.join(REPO_ROOT, 'tools/command-center/src/repohealth.js'), 'utf8');
