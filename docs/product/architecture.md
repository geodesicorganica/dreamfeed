...
| Governed memory | Product control plane sidecar | Approved contextual aid with kind/state/scope/provenance/hash; structured + keyword retrieval with visible reasons; shared inspector provenance; stable export envelope; capped assistant context with citations/warnings; exportable and non-authoritative | Auto-saved assistant truth, source replacement, secret storage, cross-project retrieval |
| Verification and release evidence | Product control plane sidecar | Approved verification snapshots and release candidates with project scope, git/ledger/lifecycle references, content hashes, guarded reads, stable export, and founder-class shipped marking | Test execution, deployment, public release creation, source-truth replacement, secret storage |
...
| Memory drift or hidden truth | Approval-gated sidecar memory, content hashes, tombstones, source/ledger precedence, visible retrieval reasons, provenance inspector, citation/conflict warnings, capped assistant context metadata | Memory lifecycle, HTTP, assistant, and UI contract tests |
| Release evidence becomes hidden truth | Sidecar records are non-authoritative; source files, Git state, and ledger remain authoritative; shipped is founder-class; exports are evidence packages only | Release lifecycle, HTTP, export, and UI contract tests |
...
| **D34 memory trust hardening** | **Implemented 2026-07-07** | Deterministic retrieval reasons, state/scope/tag filters, provenance inspector, stable export envelope, memory counts, capped assistant memory context, citations, warning metadata, and visible truncation metadata. Evidence: `test/memory.test.js`, `test/ui-contract.test.js` |
| **D35 verification/release cockpit** | **Implemented 2026-07-07** | `.dreamfeed/` schema v3 verification records and release candidates; guarded GET-only reads/exports; lifecycle-only writes; founder-class shipped marking; Release cockpit tab and provenance inspector. Evidence: `test/release.test.js`, `test/ui-contract.test.js`, `test/constraints.test.js` |
