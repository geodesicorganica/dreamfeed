...
| T03 | Preserve V1 six objects, IDs, provenance, freshness, topology, viewer protections, GET-only, localhost, memory boundary | implemented | PS-002, Gate C/F | Portable test suite (`npm test`) |
...
| T10 | Source browsing, proposed editing, diff/branch/test/deploy visibility, controlled direct instruction | reference build | Phase 1.3 scope | stakeport_os reference build; this package is read-only (GET-only) |
...
| T12 | Request → plan → review → approval → execution → immutable trace | reference build | Phase 1.3 scope | stakeport_os reference build; no execution lifecycle exists in this package |
| T13 | Least privilege, confirmation, pause/resume/halt/rollback, visual ledger | reference build | Architecture §Lifecycle | stakeport_os reference build. **Execution controls (pause/resume/halt/rollback) are prohibited in this package** under PS-002 Phase 1 / Gate F |
...
| T30 | Future persistence versioned, tenant scoped, exportable, non-authoritative | implemented | Cross-cutting requirements | Architecture §Data ownership; this package remains in-memory |
