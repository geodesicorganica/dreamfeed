...
The Dreamfeed design system lives in `docs/design-system/` at the repo root.
Read `docs/design-system/README.md` first, then explore the other files there.
...
Where to look (all paths relative to the repo root):
- `docs/design-system/README.md` — full brand context, CONTENT FUNDAMENTALS, VISUAL FOUNDATIONS, ICONOGRAPHY, and a file manifest.
- `docs/design-system/styles.css` + `docs/design-system/tokens/` — the design tokens (link `styles.css`; everything is a CSS custom property).
- `docs/design-system/assets/` — the Verified Node logo (mark, light variant, lockup, favicon).
- `docs/design-system/components/` — reusable React primitives (Button, Panel, StatePill, TraceRow, KeyValue, TextField, Select, Switch, Tabs, Badge, IconButton). Each has a `.prompt.md` with usage.
- `docs/design-system/guidelines/` — foundation specimen cards (colors, type, spacing, brand).
- `docs/design-system/ui-kits/command-center/` — an interactive recreation of the cockpit (five regions + lens registry) to copy patterns from.
